...
// pages that are not minified
output.addPage = function (pageName, result) {
    // resolve the path here
    result.js = fh.resolve(result.js, config.getSrcWebRoot());
    result.css = fh.resolve(result.css, config.getSrcWebRoot());
    pages[pageName] = result;
};

// min files are already in the dst folder
output.addMinedPage = function (pageName, result) {
    // resolve the path here
...
output.toJson = function () {
    ...
    var strPages = JSON.stringify(pages, null, 4);
